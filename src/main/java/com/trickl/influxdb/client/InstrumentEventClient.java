  ...
  */
  public Flux<Integer> store(PriceSource priceSource, List<InstrumentEvent> events) {
    Mono<Flux<Integer>> storeMarketsChangeEvents = 
        Flux.fromIterable(events).filter(event -> event instanceof MarketStateChange)
        .cast(MarketStateChange.class).collectList()
        .map(list -> marketStateChangeClient.store(priceSource, list));

    Mono<Flux<Integer>> storeSportsEventIncidents = 
        Flux.fromIterable(events).filter(event -> event instanceof SportsEventIncident)
        .cast(SportsEventIncident.class).collectList()
        .map(list -> sportsEventIncidentClient.store(priceSource, list));

    Mono<Flux<Integer>> storeSportsEventOutcomeUpdates = 
        Flux.fromIterable(events).filter(event -> event instanceof SportsEventOutcomeUpdate)
        .cast(SportsEventOutcomeUpdate.class).collectList()
        .map(list -> sportsEventOutcomeUpdateClient.store(priceSource, list));

    Mono<Flux<Integer>> storeSportsEventScoreUpdates = 
        Flux.fromIterable(events).filter(event -> event instanceof SportsEventScoreUpdate)
        .cast(SportsEventScoreUpdate.class).collectList()
        .map(list -> sportsEventScoreUpdateClient.store(priceSource, list));

    return Flux.merge(storeMarketsChangeEvents,
        storeSportsEventIncidents,
        storeSportsEventOutcomeUpdates, 
        storeSportsEventScoreUpdates).flatMap(rows -> rows);
  }
  ...
  public Flux<InstrumentEvent> findBetween(PriceSource priceSource, QueryBetween queryBetween) {
    return Flux.mergeOrdered(InstrumentEventClient::compareEventTimes, 
  ...
  protected static int compareEventTimes(
      InstrumentEvent first, InstrumentEvent second) {
